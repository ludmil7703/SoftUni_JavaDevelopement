...
public class WrongMeasurements {
        ...
        public static void main(String[] args) {
            ...
            for (int i = 0; i < matrix.length; i++) {
                matrix[i] = getInts(scanner);

            }
            int[] indexes = getInts(scanner);
        ...
        private static int getNearbySum(int[][] matrix, int r, int c, int valueToReplace) {
            ...
            if (isInBound(matrix,r+1, c) && matrix[r+1][c] != valueToReplace){
                sum += matrix[r+1][c];
            ...
            if (isInBound(matrix, r, c-1) && matrix[r][c-1] != valueToReplace){
                sum += matrix[r][c-1];
            }
            if (isInBound(matrix, r, c+1) && matrix[r][c+1] != valueToReplace){
                sum += matrix[r][c+1];
